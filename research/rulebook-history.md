# Rulebook history — the incident journal behind AGENTS.md

This file is the pre-2026-09-06 `AGENTS.md`, moved here verbatim (section headings
unchanged) so that every `AGENTS.md § "..."` / `task NNN` citation in code comments and
research docs still resolves. It is NOT loaded into agent context; the rules it produced
live in `AGENTS.md` as short imperatives, each pointing back here for the evidence.

Do not append new incidents here. New lessons go into `AGENTS.md` as a rule (see the
"Adding a rule" section there); the narrative goes into the PR that adds the rule.

Historical vocabulary: "conductor", "worker", "task file", "heartbeat", "board", "residue"
refer to an orchestration layer removed on 2026-09-05 (PR #121).
