...
function Enter-ScCustomGame {
    ...
    runs with the map and game type set, before Ok, for a suite that captures the lobby.
    Both run in a child scope: a value they must hand back goes through $script:.
    -StopAt Map returns with the map selected and the game type untouched (for the
    probes that test the combo itself); -StopAt Lobby returns after the game type and
    -BeforeStart, before Ok. -Fixtures is omitted only for a stock map this run did not
    generate. -ActivationNudge posts the activation nudge before every input, which the
    ...
    param(
        ...
        [ValidateSet('Map', 'Lobby')][string]$StopAt,
