...
# (task 026) the command-card read-back, (task 029) the upgrade queue and
# (task 030) the group production fan-out and (task 034) the widescreen patch set.
$pluginSrc = @('scplugin.cpp', 'sc_log.cpp', 'sc_hook.cpp', 'sc_fanout.cpp', 'sc_circles.cpp', 'sc_hudrow.cpp', 'sc_prodqueue.cpp', 'sc_card.cpp', 'sc_upgrades.cpp', 'sc_prodfan.cpp', 'sc_screen.cpp') |
             ForEach-Object { Join-Path $srcDir $_ }
$testSrc   = @('hooktest.cpp', 'sc_log.cpp', 'sc_hook.cpp', 'sc_fanout.cpp', 'sc_circles.cpp', 'sc_hudrow.cpp', 'sc_prodqueue.cpp', 'sc_card.cpp', 'sc_upgrades.cpp', 'sc_prodfan.cpp', 'sc_screen.cpp') |
             ForEach-Object { Join-Path $srcDir $_ }
