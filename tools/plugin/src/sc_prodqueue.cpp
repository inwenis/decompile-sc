...
//
// Everything here runs on the GAME THREAD, from one of three detours; the one exception is
// ScProdQueueLogState, called by the observer thread from the marker channel, which never
// writes.
...
// ScThreadCheck logs one line per site: the phantom bracket's safety argument is "every
// engine reader of the ring runs on the game thread" (sc_queueind.h), and those lines
// MEASURE that instead of trusting it.
// ---------------------------------------------------------------------------
// State
// ---------------------------------------------------------------------------
...
// detour runs for every producing building on every frame, so the walk runs only on the
// two RARE detours, Train and Cancel Train. The cost is latency: a building that dies
// while the player is idle is refunded on their next click, not on the next frame.
...
// could afford the item and deducted the cost before the plugin moved it out of the ring.
// The only resource write here is the REFUND, for an item destroyed while the plugin is
// holding it, and it reads the same two per-type tables the engine's own refundByType
// (0x0042CEC0) reads and honours the same units.dat "this type moves no resources" bit --
// so spend and refund cancel exactly.
// ---------------------------------------------------------------------------
...
// A record whose building has gone gives its items back, matching vanilla: the
// unit-removal path 0x0049FD00 calls cancelAllAndClearQueue (0x00466E80), which refunds
// every occupied slot before clearing the array (research/production-queue.md 4.4).
// Without this a destroyed building is a way to burn minerals.
...
// THE EPOCH TEST. It runs FIRST in every entry point this file has -- read-backs
// included, before CollectGarbage and before anything reads r->unit -- so no caller can
// observe a record from a game it does not belong to (sc_session.h, "how a module adopts
// it"). RecordStillLive cannot stand in for it: a save restores the uniqueness byte, the
// player and the hitpoints verbatim into the same seat of the same static array, so it
// answers "yes" (measured) for a record belonging to a game that has ended.
//
// NO REFUND on this path, and it is not an oversight: those minerals were taken in a
...
//
// Both directions are a bare store into the engine's own ring and move NO resources -- the
// engine paid for these items when it accepted them. Nothing else in addToBuildQueue
// applies: the cost tables it fills are consumed by its own deduction two instructions
// later, the secondary order is already set (the ring cannot be non-empty otherwise), and
// it never touches the building-AI mirror arrays.
// ---------------------------------------------------------------------------

// The slot holding the item accepted MOST RECENTLY. Occupied slots run contiguously from
// the head -- findFreeBuildQueueSlot (0x004669B0) stops at the first 0xE4, so a gap behind
// the head never occurs -- which puts the newest of `len` items at (head + len - 1) % 5.
...
// stays there, so the largest reachable logical queue is five plus whatever is held --
// exactly `g_maxTotal`. Measuring against the hold instead lets one item past the maximum.
// Leaving the ring full IS the cap: the client greys its own Train button out at five
// (research/production-queue.md 4.1), so nothing is silently swallowed or un-spent.
...
// Takes the newest items back out of the ring until it is down to the hold, appending each
// to the record in the order the engine accepted them -- which keeps the logical queue in
// the player's order, because the only item above the hold is the one just arrived.
// `rp` may point at a NULL record: one is created on the first item actually held, so an
// untouched building never occupies a slot in the table.
...
// plugin has anything to give it, and never more while the plugin can take the excess.
// Hold first, then promote -- the other order puts an item back only to take it out again.
...
void ScProdQueueOnTrain(DWORD unit, unsigned type, bool wasFull) {
    ...
    do {
        ...
        if (wasFull) {
            // The ring was already full when the command arrived, so the engine dropped it
            // at its own `CMP EAX,0x5` without touching the array or the player's
            // resources -- reachable only at the cap, so the item is refused, not lost.
        ...
        // ring stays below the five at which the client stops sending.
...
void ScProdQueueOnTick(DWORD unit) {
    ...
    // producing building on every frame. The sync then happens on the next tick with
    // something to look at, or on any Train/Cancel/read-back, which all take it always.
...
bool ScProdQueueOnCancel(DWORD unit, unsigned payload) {
    ...
    // cancelLastQueued at 0x00466E40) can be ours, and only while the plugin holds the tail
    // of this building's logical queue. A specific slot 0..4 or the 0xFF no-op is the
    // engine's, and it refunds those itself.
    ProdRecord* r = ScLedgerFind(g_rec, g_recCount, unit);
    if (r && r->count > 0 && payload == SC_CANCEL_TRAIN_LAST) {
        ...
        // which refunds `buildQueue[(head + k) % 5]` (research/production-queue.md 8.1).
        // The test above is that same arithmetic on the building's own memory: a slot
        // holding 0xE4 means the icon is NOT drawing an engine item, so handing the click
        // through would refund BY THE SENTINEL TYPE 228 -- two cost tables read out of
        // bounds, crediting the player whatever is there. Vanilla can never produce such a
        // click (an empty slot's icon is drawn DISABLED and both engine input paths refuse
        // a disabled control, research/command-card.md 5); the queue indicator can, because
        // it draws those icons from the plugin's own overflow. So the plugin owns every such
        // click: it cancels its own item, or swallows a click it has no item for.
...
void ScProdQueueLogState(const char* tag) {
    ...
    // The observer's own site: EXPECTED to print a different id from the
    ...
    EnterCriticalSection(&g_lock);
    // The oracle syncs too: a mechanism that only dropped stale records on the next Train
    // would still print them in `buildings=` here, reading as no fix at all.
    ProdQSessionSync();

    // The SOLE SELECTED building, tracked or not: without it the oracle is silent exactly
    // when the plugin holds nothing, making "holding nothing" and "the oracle did not run"
    ...
    // ALWAYS a summary line, even with zero records -- an absence has to be positively
    // reported (AGENTS.md § "Oracles: absence and defect-era checks"). The detour counters
    // keep "holding nothing" apart from "never asked", which print the same zeros; they go
    // at the END of the line so existing parsers keep working. Every field is a count some
    // path increments, never a printed constant (AGENTS.md § "Diagnostics and reporting").
...
void ScProdQueueLogStats(void) {
    if (!g_enabled) return;
    // Refund() moves the refund fields, which is how a double refund or a swallowed item is
    // told apart from a balance that merely looks plausible. staleSession= is on the same
    // terms: ProdQSessionSync is the only path that moves it, and hooktest asserts it.
...
// Detours
//
// The building a production command acts on is the ONE unit selected: both receive handlers
// reset selectionIterator (0x006284B6) and then require getActivePlayerNextSelection to
// yield exactly one unit (research/production-queue.md 2.2 quotes both prologues).
//
// WHICH SELECTION ARRAY THAT IS matters: two twelve-slot arrays ABUT, so reading the wrong
// one costs nothing until the day they disagree. `getActivePlayerNextSelection`
// (0x0049A850) walks `playersSelections`, indexed by the ACTIVE PLAYER -- NOT the abutting
// `activePlayerSelection` (0x006284B8). Its own index arithmetic
// (research/production-queue.md 2.2, from this binary):
//
...
//
// The two agree whenever the player has ONE building selected, so reading the wrong one
// passes every single-building test. They disagree when the fan-out replays a Select+Train
// pair for a GROUP: the simulation's selection is moved to one building at a time (that is
// what makes cmdrecvTrain's single-unit gate accept at all), while `activePlayerSelection`
// still holds the client's whole group -- `sel[1]` non-null, so nothing is held back, every
// ring fills to five and the client stops sending. MEASURED: nine presses with three
// Command Centers boxed put FIVE 0x1F on the wire and left all three rings at 5/5, zero
// overflow. So read the array the ENGINE'S OWN GATE reads, with its index arithmetic -- the
// state the action runs in, not the state the player's screen is in
...
// Verified prologues, dumped out of the working-copy binary and quoted in
// research/production-queue.md 6.1. ScHookInstall refuses to patch if memory disagrees.
...
int ScProdQueueInstall(BYTE* moduleBase) {
    ...
    // One suspension for all three: the game is quiescent for microseconds instead of once
    // per hook, and a partially installed set is never observable from a game thread.
...
void ScProdQueueRemove(void) {
    ...
    if (g_lockReady) {
        EnterCriticalSection(&g_lock);
        // Epoch test before the refund: a record left over from a game that has ended
