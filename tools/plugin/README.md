...
| `sc_session.cpp` | the game-session epoch | `ScSession*` | `SESSION` | — |
| `sc_fanout.cpp` | command fan-out, control groups, building groups | `ScFanout*`, `ScMode` | `CMD` `FANOUT` `SHADOW` `SELECT` `SORT` `GROUP` `GROUPSTATS` `BGROUP` `UNITSTATE` | `SCPLUGIN_MODE`, `SCPLUGIN_FANOUT_CMDS`, `SCPLUGIN_CIRCLES`, `SCPLUGIN_HUDROW` |
| `sc_circles.cpp` | selection circles for the units past 12 | `ScCircles*` | `CIRCLES` | `SCPLUGIN_CIRCLES` |
| `sc_hudrow.cpp` | paging the HUD selection row | `ScHudRow*` | `HUDROW` | `SCPLUGIN_HUDROW` |
...
| `sc_stormpresent.cpp` | copying the widened strip to the primary every frame | `ScStormPresent*` | `STORM` `STORMTIME` `STORMSTALL` `STORMSTATS` | `SCPLUGIN_STORM_PRESENT` |
...
| short | long |
|---|---|
| `prodq`, `PRODQ` | production queue (task 025) |
| `prodfan`, `PRODFAN` | production **fan-out** — one click, every building in the group (task 030) |
| `queueind`, `QIND` | queue **indicator** — the "+3" for items the five-slot strip cannot show (task 033) |
| `upgq`, `UPGQ` | upgrade queue (task 029) |
| `hudrow`, `HUDROW` | the HUD **selection row** — the wireframe strip, and paging it (task 017) |
| `bgroup`, `BGROUP` | **building groups** — a drag box that selects more than one building (task 024) |
| `WS` | **widescreen** (task 034). `SC_WS_*` is generated by `tools/renderer_patch_sites.py` |
| `ctrace`, `CTRACE` | console **trace** — who claims a click at a given x (task 073) |
| `hk`, `Tramp` | a `ScHook` instance / its **trampoline** (the relocated prologue) |
| `VA` | a **virtual address at StarCraft.exe's preferred image base** (`0x00400000`). Everything in `sc_addresses.h` is one; `ScRuntimeAddr()` turns it into where it actually is |
| `Rt` | *(gone)* — the old per-module spelling of `ScRuntimeAddr` |
...
## Building groups: how a box selects more than one building

Full derivation in [`research/building-groups.md`](../../research/building-groups.md).

Vanilla selects **one** building per drag box, and one predicate does it —
`unit_IsStandardAndMovable` (`0x0047B770`), consulted on both sides of the selection path:

| where | what it does | symptom |
|---|---|---|
| `SortAllUnits` (`0x0046F0F0`), client | drops every candidate that fails it; if that emptied the list, puts the **last** one it dropped back and returns a count of 1 (`0x0046F223`, then `0x0046F27A`..`0x0046F281`) | a box over six Supply Depots selects one |
| `addUnitToSelectionSlot` (`0x0049AF80`), sim | refuses every slot but the first to a unit that fails it (`0x0049AF97`..`0x0049AFA4`) | `playersSelections[player]` — what every order applier iterates — holds one building whatever arrives on the wire |

The plugin relaxes the **client** gate for exactly one case and turns the **simulation** gate into a
number.

**The client half** rides the `SortAllUnits` detour the fan-out already installs. After the original
returns it appends the rest of the group — same unit type, same owner — to the caller's twelve
slots, and only when *all* of: mode `fanout`, `%SCPLUGIN_BUILDING_GROUPS%` on, `clicked == 0` (which
only the drag-box caller `0x0046FA40` passes), the original returned exactly 1, and that one unit
fails the predicate. Anything past the twelfth goes into the same overflow accumulator
`sortOverflowHandler` fills. The engine then does the rest itself: `0x0049AE40` attaches a selection
circle to each, the HUD row fills, `CMDACT_Select` sends all of them. **Nothing here writes
`CSprite::selectionIndex` or sets sprite flag `0x08`** — this is an ordinary N ≤ 12 selection as far
as the engine is concerned.

**The simulation half is not patched.** `0x0049AF80`'s first five bytes contain a short `JZ` and
`ScHookInstall` copies prologue bytes verbatim with no relocation, so a detour there would corrupt
its own trampoline; the function is also inlined into `CMDRECV_ShiftSelect`. Instead the gate
becomes `simSlots` — *how many of this selection the simulation will hold at once*, recomputed at
every commit: 12 normally, **1** for a building group. The fan-out chunks by that number instead of
by the constant 12, so a rally to six Barracks goes out as six `Select(1)` + `RightClick` pairs,
each accepted at slot 0, the one slot the gate never guards. With `simSlots == 12` the chunking is
byte-for-byte what it was. The visible chunks are emitted **backwards** so the last `Select` carries
the unit the engine itself put in slot 0 — with one visible chunk that is a no-op, and with
`simSlots == 1` it is what leaves the simulation holding the building the engine chose.

Scope, all asserted in game rather than described: a **mixed** box of units and buildings is stock
(the units win, and the fallback never runs); a mixed box of **two building types** takes vanilla's
own arbitrary lead and widens it to that lead's type, so no second arbitrary rule is added; single
click, shift-click, ctrl+click and double-click are stock (`clicked != 0`); shift+box is stock
(`combineSelectionsLists` applies the same predicate to both sides); and SINGLE-gated commands still
act on one building, exactly as they do for a >12-unit selection. `%SCPLUGIN_BUILDING_GROUPS%=0` /
`-BuildingGroups 0` is the off switch and the control arm of the in-game run.
