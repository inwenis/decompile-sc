...
# The per-geometry #defines. Kept as macros so the .ps1 readers (drive-game.ps1
# Get-ScWideGeometry, deploy.ps1) can regex them out of the preset's own file, and
# #undef'd at the end of each file so several presets can share one translation unit.
GEOM_DEFINES = [
...
def emit_header(b: Builder, path: str):
    ...
    out.append("")
    out.append("#ifndef SC_SCREEN_PATCH_COMMON_H\n#define SC_SCREEN_PATCH_COMMON_H\n")
    ...
    out.append("// One geometry preset: its numbers and its table. sc_screen_presets.h lists them.")
    out.append("typedef struct ScScreenGeometry {")
    out.append("    const char*          name;       // as %SCPLUGIN_WS_GEOMETRY% spells it, e.g. \"1280x880\"")
    out.append("    int                  w, h;       // the screen")
    out.append("    int                  pfW, pfH;   // the playfield (the console takes the rest)")
    out.append("    int                  gridCols, gridRows, gridBytes;")
    out.append("    int                  terrainPitch, terrainSize, terrainRows;")
    out.append("    int                  consoleShiftY;   // PF_H - 400; 0 at the stock playfield height")
    out.append("    const ScScreenPatch* patches;")
    out.append("    size_t               patchCount;")
    out.append("} ScScreenGeometry;\n")
    out.append("#endif  // SC_SCREEN_PATCH_COMMON_H\n")
