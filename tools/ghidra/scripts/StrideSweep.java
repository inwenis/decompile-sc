...
//
// In `LEA EDX,[EDI+EDI*0x2]; LEA EAX,[EBX+EDX*0x4]; MOV [EAX*0x4 + 0x6284e8],ESI` the two LEAs are
// invisible to both other sweeps: they name no global for the xref sweep and hold no watched
// constant for the immediate sweep. Yet the 12-dword (48-byte) row stride of playersSelections is
// carried entirely by the *0x2 and *0x4 scale factors, so every one of these instructions must
// change to widen the array. Every multiplier is emitted with its value so filtering happens in
// the open in build-stride-table.ps1. x3 LEAs serve every 3-, 6-, 12- and 24-byte structure in any
// binary, so `elsewhere` rows are noise: counted, not committed.
//
// Script args:
...
public class StrideSweep extends GhidraScript {
    ...
    /**
     * `LEA rD,[rA + rA*0x2]` -- the compiler's x3, and the only way to get an odd multiplier out
     * of the scaled-index addressing modes, so it is the seed. Group 1 = rD, group 2 = rA.
     */
    ...
    public void run() throws Exception {
        ...
        // getFunctions() yields only functions with a body, getFunctionCount() also counts
        // externals; reporting one alone would look like the sweep skipped part of the program.
    ...
    /**
     * Follows {@code reg} forward, multiplying in every scale factor applied to it. Conservative
     * by design: it gives up rather than guess -- a CALL clobbers caller-saved registers, so the
     * chain cannot be trusted across one -- and records why in {@code terminated}, so a chain
     * reported with a stride is one whose whole arithmetic was seen.
     */
